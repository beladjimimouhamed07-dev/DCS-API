...
local function getMissionUnitIsAirDefence_(missionUnitType)
	...
	if nil == result then
		...
		if dbUnit then
			if 	base.findAttribute(dbUnit.attribute, "Air Defence") or
				base.findAttribute(dbUnit.attribute, "Armed Air Defence") then
...
function processPlayerScore(amount)
	playerScore_ = playerScore_ + amount
end
...
function getUnitsDatabase()
	return unitsDatabase_
end	