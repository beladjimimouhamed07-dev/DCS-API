...
function LuaExportOnHumanStart()
	--mark  birth  of player's  unit   
	--local  MFD =  GetIndicator(1)
	--if  MFD then 
	--	MFD:assign_dedicated_viewport(0,0,1024,768)
	--end
end

function LuaExportOnHumanStop()
	--mark  death of player's unit
end
...
--for test with SFL build , not enabled yet for  general  public release 
--[[
	-- Supported cameras for Force Camera Request
	local camera_names =
	{
		"CameraCockpit",				-- F1
		"CameraNearPlane",				-- F2
		"CameraTower",					-- F3
		"CameraChasePlane",				-- LCtrl-F4
		"CameraNearWeapon",				-- F6
		"CameraNearGround",				-- F7
		"CameraNearNavy",				-- F9
		"CameraAWACS",					-- F10
		"CameraFree",					-- F11
		"CameraStatic",					-- F12
		"CameraChaseWeapon",			-- LCtrl-F6
		"CameraCustom",					-- F4
		"CameraWingman",				-- LAlt-F4
		"CameraObject",					-- RCtrl-F2
	}
--]]
--[[
	-- camera request table example
	request = {
		name = "CameraNearPlane",
		id = object_id,		-- for "CameraFree" used as a padlock object id
		pos = {		-- camera position
			x = {x = 1.0, y = 0.0, z = 0.0},	-- x-axis
			y = {x = 0.0, y = 1.0, z = 0.0},	-- y-axis
			z = {x = 0.0, y = 0.0, z = 1.0},	-- z-axis
			p = {x = 0.0, y = 0.0, z = 0.0},	-- point
		},
		fov = camera_fov,	-- grad, -1.0 to ignore fov in LoForceCamera
		valid_pos = true,	-- false to ignore pos in LoForceCamera
	}
--]]
--[[			
local function printCameraRequest(request, t)
	if default_output_file then
		default_output_file:write(string.format("LoCreateCameraRequest: t = %f\n", t))
		default_output_file:write(string.format("name: %s\n", request.name))
		default_output_file:write(string.format("id: %d\n", request.id))	-- object id
		default_output_file:write(string.format("pos: \nx:(x:%f, y:%f, z:%f), \ny:(x:%f, y:%f, z:%f), \nz:(x:%f, y:%f, z:%f), \np:(x:%f, y:%f, z:%f)\n", 
			request.pos.x.x, request.pos.x.y, request.pos.x.z, 
			request.pos.y.x, request.pos.y.y, request.pos.y.z, 
			request.pos.z.x, request.pos.z.y, request.pos.z.z, 
			request.pos.p.x, request.pos.p.y, request.pos.p.z ))
		default_output_file:write(string.format("fov: %f\n", request.fov))	-- field of view (grad), -1.0 -> ignore in LoForceCamera
		default_output_file:write(string.format("valid_pos: %s\n", tostring(request.valid_pos)))	-- false -> ignore pos in LoForceCamera
	end
end	
--]]

-- 3) Force camera request
--[[
	-- create request from current camera
	local request = LoCreateCameraRequest()
	printCameraRequest(request, t)
--]]
--[[
	-- set camera directly from a request
	LoForceCamera(request)	
--]]

-- 4) Camera bookmarks
-- user bookmarks can be created only for CameraFree, CameraObject, CameraCustom
--[[
	-- CameraObject bookmarks example
	if t == 0.0 then
		local o = LoGetWorldObjects()
		for k,v in pairs(o) do
			if default_output_file then
				default_output_file:write(string.format("t = %.2f, ID = %d, name = %s, country = %s(%s)\n", t, k, v.Name, v.Country, v.Coalition))
			end
			if v.Name == "A-10C" then
				-- Force to CameraObject for A-10C unit 
				local request = LoCreateCameraRequest()
				request.id = k
				request.name = "CameraObject"
				request.fov = -1.0
				request.valid_pos = false
				LoForceCamera(request)
				tNext = tNext + 2.0
				break
			end
	...
	elseif t == 2.0 then
		bookmarks = LoGetUserBookmarks() -- for current camera and for current unit type
		if bookmarks then
			if default_output_file then
				default_output_file:write(string.format("LoGetUserBookmarks: t = %f\n", t))
				for k,v in ipairs(bookmarks) do
					default_output_file:write(string.format("%d: name = %s\n", k, v.name))
					default_output_file:write(string.format("pos: \nx:(x:%f, y:%f, z:%f), \ny:(x:%f, y:%f, z:%f), \nz:(x:%f, y:%f, z:%f), \np:(x:%f, y:%f, z:%f)\n", 
						v.pos.x.x, v.pos.x.y, v.pos.x.z, 
						v.pos.y.x, v.pos.y.y, v.pos.y.z, 
						v.pos.z.x, v.pos.z.y, v.pos.z.z, 
						v.pos.p.x, v.pos.p.y, v.pos.p.z ))
					default_output_file:write(string.format("fov: %f\n", v.fov))	-- field of view (grad)	
				end
			end
			if #bookmarks > 0 then
				bookmark_index = 1
	...
	elseif t > 2.0 and bookmarks and bookmark_index <= #bookmarks then
		local bookmark = bookmarks[bookmark_index]
		if default_output_file then
			default_output_file:write(string.format("t = %f, bookmark %d: name = %s\n", t, bookmark_index, bookmark.name))
		end	
		local request = LoCreateUserBookmarkRequest(bookmark.name)
		if bookmark_index < #bookmarks then
			bookmark_index = bookmark_index + 1
			tNext = tNext + 2.0
		end
	end
--]]
--[[
	-- CameraFree bookmarks example
	if t == 0.0 then
		-- to CameraFree
		local request = LoCreateCameraRequest()
		request.id = 0
		request.name = "CameraFree"
		request.fov = -1.0
		request.valid_pos = false
		LoForceCamera(request)
		tNext = tNext + 2.0
	elseif t == 2.0 then
		bookmarks = LoGetUserBookmarks()
		if bookmarks then
			if default_output_file then
				default_output_file:write(string.format("LoGetUserBookmarks: t = %f\n", t))
				for k,v in ipairs(bookmarks) do
...
-- 5) Force remote camera request in MP game
	--LoGetLocalPlayer()	-- returns player table { host_id, name }
	--LoGetPlayers()		-- returns all players table { { host_id, name }, ... }
	--LoSetAllowRemoteForceCameraRequests(true/false)	-- false by default, set true to process incoming remote camera requests locally	
	--LoSendForceCamera(host_id, request)	-- send camera request to host_id ( 0 -> to all other hosts )		
--[[	
	if t == 0.0 then
		LoSetAllowRemoteForceCameraRequests(true)		
		tNext = tNext + 2.0 
	elseif t == 2.0 then
		players = LoGetPlayers()
		default_output_file:write("players:\n")
		for k,p in ipairs(players) do
			default_output_file:write(string.format("[%d] = { host_id = %d, name = %s },\n", k, p.host_id, p.name))
		end
		local_player = LoGetLocalPlayer()
		default_output_file:write(string.format("local_player = { host_id = %d, name = %s }\n", local_player.host_id, local_player.name))
		tNext = tNext + 2.0 
	elseif t == 4.0 then
		request = LoCreateCameraRequest()
		local host_id = 0	-- to all others
		LoSendForceCamera(host_id, request)		
	end
--]]	
