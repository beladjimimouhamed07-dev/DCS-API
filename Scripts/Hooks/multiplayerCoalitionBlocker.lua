...
function onPlayerTryChangeCoalition(player_id, side)
    ...
    if DCS.isTrackPlaying() == true or params.isActive == false or side == 0 then
        return true
    end
...
function onPlayerChangeCoalition(player_id, side)
    if params.isActive == false or side == 0 or DCS.isServer() == false or DCS.isTrackPlaying() == true then
        return
    end
...
function onMissionLoadEnd()
    if params.isActive == false or DCS.isServer() == false or DCS.isTrackPlaying() == true then
        return
    end
