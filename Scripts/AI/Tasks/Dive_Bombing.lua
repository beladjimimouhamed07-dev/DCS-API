...
WINGOVER_DIVE_ENTRANCE = 4

--local dive_bombing_logic = ONLY_HOR_AIMING_AND_DIVING
local dive_bombing_logic = WINGOVER_DIVE_ENTRANCE
...
Conditions = {
	...
	IF_PLANE_AHEAD = 17,
	ON_WINGOVER_DIVE_START_POSITION = 18,
	IF_DIVE_ENTRANCE_COMPLETE = 19,
	IF_WINGOVER_FORBIDEN = 20,
	IF_EGRESS_FINISHED = 21,
}
...
States = {
	...
	BREAKAWAY = 10,
	MOVE_TO_DIVE_START = 11,
	WINGOVER_DIVE_START = 12,
	EGRESS = 13,
}
...
Subtasks = {
	...
	MOVING_TO_DIVE_START_POINT = 18,
	WINGOVER_DIVE_ENTRANCE = 19,
	EGRESS_COURSE = 20,
}
...
SubtasksByState = {
	...
	[States.MOVE_TO_DIVE_START] = {Subtasks.VELOCITY_HOLDING,Subtasks.MOVING_TO_DIVE_START_POINT},
	[States.WINGOVER_DIVE_START] = {Subtasks.MIN_VELOCITY_HOLDING,Subtasks.WINGOVER_DIVE_ENTRANCE},
	[States.EGRESS] = {Subtasks.EGRESS_COURSE},
}
...
elseif dive_bombing_logic == WINGOVER_DIVE_ENTRANCE then
	SubtasksByState[States.AIMING] = {Subtasks.VELOCITY_HOLDING, Subtasks.HOR_AIMING, Subtasks.DIVING}
	SubtasksByState[States.AIMING_AND_BOMBING] = {Subtasks.VELOCITY_HOLDING, Subtasks.HOR_AIMING, Subtasks.DIVING, Subtasks.BOMBING2}
	SubtasksByState[States.ALTITUDE_STABILIZING_AND_BOMBING] = {Subtasks.VELOCITY_HOLDING, Subtasks.ATTACK_COURSE_HOLDING, Subtasks.ALTITUDE_STABILIZING, Subtasks.BOMBING2}
end
	...
	FSM = {
		...
		[States.APPROACHING] = {
			{condition = {operator = "and", 
							operands = {{operator = "not", operand = Conditions.IF_WINGOVER_FORBIDEN}, 
										{subtask = Subtasks.APPROACHING, status = SubtaskStates[Subtasks.APPROACHING].FINAL}, 
										Conditions.IF_HIGHER_THAN_TARGET }},
				new_state = States.MOVE_TO_DIVE_START},
			{condition = {operator = "and", 
							operands = {Conditions.IF_WINGOVER_FORBIDEN,
										{subtask = Subtasks.APPROACHING, status = SubtaskStates[Subtasks.APPROACHING].FINAL},
										Conditions.IF_HIGHER_THAN_TARGET }},
				new_state = States.HOR_AIMING},
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY}
		},
		[States.MOVE_TO_DIVE_START] = {
			{condition = Conditions.ON_WINGOVER_DIVE_START_POSITION, new_state = States.WINGOVER_DIVE_START},
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY},
		},
		[States.WINGOVER_DIVE_START] = {
			{condition = {operator = "and", operands = {Conditions.IF_MINIMAL_ROLL, Conditions.IF_DIVE_ENTRANCE_COMPLETE}}, new_state = States.AIMING_AND_BOMBING},
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY},
			{condition = Conditions.IF_GROUND_PROXIMITY, new_state = States.BREAKAWAY}
		},
		...
		[States.AIMING_AND_BOMBING] = {
			{ condition = {operator = "and", operands = {Conditions.IF_LINE_RELEASE, {subtask = Subtasks.BOMBING2, status = SubtaskStates[Subtasks.BOMBING2].RELEASE_IN_PROCESS}}}, new_state = States.ALTITUDE_STABILIZING_AND_BOMBING},
			{ 	condition = {operator = "or", operands = {{subtask = Subtasks.BOMBING2, status = SubtaskStates[Subtasks.BOMBING2].RELEASE_FINISHED}, Conditions.ON_BREAKAWAY_POSITION}},
				new_state = States.EGRESS},
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY},
			{condition = Conditions.IF_TURN_TO_TARGET_IMPOSSIBLE, new_state = States.BREAKAWAY}
		},
		[States.ALTITUDE_STABILIZING_AND_BOMBING] = {
			{condition = {subtask = Subtasks.BOMBING2, status = SubtaskStates[Subtasks.BOMBING2].RELEASE_FINISHED}, new_state = States.EGRESS},
			{condition = Conditions.ON_BREAKAWAY_POSITION, new_state = States.EGRESS},
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY},
		},
		[States.ALTITUDE_STABILIZING] = {
		...
		[States.EGRESS] = {
			{condition = Conditions.IF_PLANE_AHEAD, new_state = States.MID_AIR_COLLISION_AVOIDANCE, action = 'PUSH' },
			{condition = {operator = "and", operands = {Conditions.IF_MINIMAL_ROLL, Conditions.IF_EGRESS_FINISHED}}, new_state = States.BREAKAWAY},
			{condition = Conditions.IF_TARGET_DESTROYED, new_state = States.BREAKAWAY}
		},
	}
end
