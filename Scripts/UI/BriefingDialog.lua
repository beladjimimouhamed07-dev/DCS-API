...
function DTC_onChange()
    hide()
    DCS.toggleDTC(unitType, true, nil, nil, "BriefingDialog")
end
...
function updateStartGroups(a_tblStartData)
    ...
    if a_tblStartData.airdromeId then
        ...
        if a_tblStartData.ID then
            local posX, posY, posZ = DCS.getUnitPositionByObjectId(a_tblStartData.ID)
...
--		base.print("--dataBrf.side--",dataBrf.side)
    ...
    table.insert(autoBriefing, composeEntry(nil, _('QNH_brief','QNH'),    string.format('%0.2f %s/ %d %s/ %0.2f %s', (dataBrf.qnh or 0) / 0.7500637554192,_('hPa'), dataBrf.qnh or 0, _('mmHg'), (dataBrf.qnh or 0) / 25.4, _('inHg'))))
...
function checkAvailabilityDTC()
    ...
    if unitTypeDesc ~= nil and unitTypeDesc.DTC then
        ...
        if DCS.getAvailableSlots(unitCoal) ~= nil then
