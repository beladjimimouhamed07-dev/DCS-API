...
function create()
	...
	window_.containerMain.pDown.btnExportToMiz:setVisible(false)
...
function setInitiatorMissionIdListener(listener, combolist)
	...
	function listener:onInitiatorMissionIdChange(id)
		...
		if rockets and rockets[id] and rockets[id].display_name then
			unitName = rockets[id].display_name
		end
...
function getEventDetails(event)
	...
	if weaponName and event.failure == nil then
		...
		if hitCount and hitCount > 1 then
			...
			local bdaCount = DebriefingEventsData.getEventBdaCount(event)
			
			if bdaCount and bdaCount > 1 then
				details = string.format('%s (%d)', weaponName, bdaCount)
			end
			
			local endShootingCount = DebriefingEventsData.getEventEndShootingCount(event)
			if endShootingCount and endShootingCount > 1 then
				details = string.format('%s (%d)', weaponName, endShootingCount)
			end
		end
	else
...
function addEventToGrid(event)
	...
	elseif event.target then
		...
		if rockets then 
			local rocket = rockets[event.target]
			if rocket and rocket.display_name then
				targetName = rocket.display_name
			else
...
function getRockets()
	if rocketsByName == nil then
		if base.rockets then
			rocketsByName = {}
			for k,v in base.pairs(base.rockets) do
				if v.name then
					rocketsByName[v.name] = v
				end	
			end
			return rocketsByName
		end
	else
		return rocketsByName
	end
