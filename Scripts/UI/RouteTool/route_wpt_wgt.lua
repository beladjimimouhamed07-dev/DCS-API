...
function onChangeRightButton(self)
	...
	if item then
		...
		item = self.parent.CenterCombo:getItem(0)  
	end	
	if item then
...
function setWPT(self, a_index, a_maxIndex)
    local a_startIndex = 1
    local a_endIndex = a_maxIndex
    ...
    for i=1,a_maxIndex do
        if (i >= a_startIndex) and (i <= a_endIndex)then
