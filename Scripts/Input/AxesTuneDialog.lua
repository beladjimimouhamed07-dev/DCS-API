...
local function fillComboListAxes_(self, combos, deviceName)
	self.comboListAxes_:clear()
	
	for i, combo in ipairs(combos) do
		local localizedComboName = InputUtils.createComboString(combo, deviceName)
		local item = ListBoxItem.new(localizedComboName)
		item:setTooltipText(localizedComboName)

		item.axis = combo.key
		item.reformers = combo.reformers
...
local function setFilterHardwareABDetent(self, isHardwareABDetent, hardwareDetentMax, hardwareDetentAB)
	...
	if ED_PUBLIC_AVAILABLE then
		self.checkBoxHardwareABDetent_:setVisible(false)
		self.panelHardwareABDetent_:setVisible(false)
	else
...
local function show(self, combos, deviceName)	
	...
	fillComboListAxes_(self, combos, deviceName)
