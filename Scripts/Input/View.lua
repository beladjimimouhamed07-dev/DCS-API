...
local cdata = {
	...
	commandComboConflict	= _('The assigned combo have conflicts'),
}
...
local function setComboCell(widget, deviceName, rowInfo)
	...
	InputUtils.setCellTexts(widget, rowInfoCell, getDeviceDisabled_(deviceName))
...
local function updateActionCell_(rowInfo)
	...
	if rowInfo.nameDuplicated then
		...
		if rowInfo.valid == true then
			tooltipText = rowInfo.nameHint
		else
			tooltipText = rowInfo.nameHint.."\n"..cdata.commandComboConflict
		end
	end
...
local function findRows(self, searchFunc)
	local result = {}
	
	for i, rowInfo in ipairs(self.currentRowInfos_) do
		if searchFunc(rowInfo) then
			table.insert(result, {resultIndex = i - 1, resultRowInfo = rowInfo})
		end
	end
	...
	for i, data in ipairs(result) do
		if self.currentRowInfoIndices_ then
			-- resultRowInfo может отсутствовать среди видимых строк
			data.resultIndex = nil
			
			for i, rowInfoIndex in ipairs(self.currentRowInfoIndices_) do
				if data.resultRowInfo == self.currentRowInfos_[rowInfoIndex] then
					data.resultIndex = i - 1
					break
				end
			end
		end
...
function activate_(self)		
	Input.process()
	createInputLayer_(self, self:getCurrentProfileName())
	Input.setTopLayer(inputLayerName)
...
local function findComboRows_(self, combo, deviceName)
	local comboHash = InputUtils.getComboHash(combo.key, combo.reformers)

	return findRows(self, function(rowInfo)
		local rowInfoCell = rowInfo.cells[deviceName]
...
local function addCombo_(self)
	if self.activeColumn_ and self.activeRow_ then
		...
		if rowInfoCell and not rowInfoCell.disabled and not rowInfoCell.noCommand then
			...
			exit_dialog.setNeedHideHandler("addComboDialog", addComboDialog)
			local combo = addComboDialog:show(rowInfo.name, isAxisCommands, deviceName, profileName)
			exit_dialog.setNeedHideHandler("addComboDialog", nil)
			
			if combo then
				local command
				
				-- комбо могло принадлежать другой команде
				local prevComboCommands = {}
				local searchResult = findComboRows_(self, combo, deviceName)
				...
				if isAxisCommands then
					...
					for i, infoData in ipairs(searchResult) do
						if infoData.resultRowInfo then
							table.insert(prevComboCommands, InputData.getProfileAxisCommand(profileName, infoData.resultRowInfo.hash))
						end
					end
				else
					...
					for i, infoData in ipairs(searchResult) do
						if infoData.resultRowInfo then
							table.insert(prevComboCommands, InputData.getProfileKeyCommand(profileName, infoData.resultRowInfo.hash))
						end
					end
				end				
				...
				for i, prevComboCommand in ipairs(prevComboCommands) do
					setRowInfoCommand(searchResult[i].resultRowInfo, prevComboCommand)
					
					if searchResult[i].resultIndex then
						updateComboCell_(self, self.activeColumn_, searchResult[i].resultIndex)
						updateActionCell_(searchResult[i].resultRowInfo)
						updateCategoryCell_(searchResult[i].resultRowInfo)
					end	
				end
...
local function removeCommandDeviceCombos_(self)
	if self.activeColumn_ and self.activeRow_ then
		...
		if rowInfoCell then
			...
			for i, combo in ipairs(rowInfoCell.combos) do
				local prevComboCommands = {}
				local searchResult = findComboRows_(self, combo, deviceName)
				...
				for i, prevComboCommand in ipairs(prevComboCommands) do
					prevComboCommand.updated = false
					InputData.updateCommandValidation(profileName, deviceName, prevComboCommand, rowInfo.isAxisCommand)
					setRowInfoCommand(searchResult[i].resultRowInfo, prevComboCommand)
					updateActionCell_(searchResult[i].resultRowInfo)
					updateCategoryCell_(searchResult[i].resultRowInfo)
					if searchResult[i].resultIndex then
...
local function setDefaultCommandDeviceCombos_(self, rowInfo, rowIndex, profileName, deviceName)
	...
	if combos then 
		for i, combo in ipairs(combos) do
			...
			for i, infoData in ipairs(searchResult) do
				if infoData.resultRowInfo.hash ~= rowInfo.hash then
					table.insert(prevComboRowInfos, {rowIndex = infoData.resultIndex, rowInfo = infoData.resultRowInfo})
				end
			end
		end
	end
...
local function setModifiers_(self)
	...
	local exit_dialog = require('me_exit_dialog')
	exit_dialog.setNeedHideHandler("modifiersDialog", modifiersDialog)	
	local modifiers = modifiersDialog:show(profileName)
	exit_dialog.setNeedHideHandler("modifiersDialog", nil)	
...
local function tuneAxes_(self)
	if self.activeColumn_ and self.activeRow_ then
		...
		if rowInfoCell then
			...
			local exit_dialog = require('me_exit_dialog')			
			exit_dialog.setNeedHideHandler("axesTuneDialog", axesTuneDialog)
			local filters = axesTuneDialog:show(rowInfoCell.combos, deviceName)
			exit_dialog.setNeedHideHandler("axesTuneDialog", nil)
...
local function tuneForceFeedback_(self)
	if self.activeColumn_ then
		...
		if 'joystick' == deviceTypeName then
			...
			local exit_dialog = require('me_exit_dialog')
			exit_dialog.setNeedHideHandler("forceFeedbackTuneDialog", forceFeedbackTuneDialog)	
			local result = forceFeedbackTuneDialog:show(settings)
			exit_dialog.setNeedHideHandler("forceFeedbackTuneDialog", nil)	
...
local function construct(self, container, a_w, a_h)
	...
	self.container_.panelTop:setSize(a_w-560, 28)	
	...
	container.panelRenameColumn.editBox:addKeyUpCallback(function (self,keyName, unicode)
			if keyName == 'return' or keyName == 'escape' then
				bDownKeyInEditBox = false
	...
	self.buttonWizard_:addChangeCallback(function()
		...
		local profileName = self:getCurrentProfileName(self)
		--Wizard.show(profileName, function()
		--	self.setCurrentProfileName(self, profileName)
		--end)
		InputVisualizerDialog.onShowManual(profileName, function()
			self.setCurrentProfileName(self, profileName)
		end)
...
local function isEditEditBox()
	return bDownKeyInEditBox
end
