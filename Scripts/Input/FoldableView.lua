...
local function setComboCell(widget, deviceName, rowInfo)
	...
	InputUtils.setCellTexts(widget, rowInfoCell , getDeviceDisabled(deviceName))
...
local function findComboRowsInfo(self, combo, deviceName)
	local result = {}
...
local function resetComboToDefault(self, rowInfo, profileName, deviceName)
	...
	if combos then 
		for i, combo in ipairs(combos) do
			local comboRowInfos = findComboRowsInfo(self, combo, deviceName)
			
			for i, comboRowInfo in ipairs(comboRowInfos) do
...
function activateInput(self)		
	Input.process()
	createInputLayer(self, self:getCurrentProfileName())
	Input.setTopLayer(inputLayerName_)
...
local function addCombo(self)
	if self.activeColumn_ and self.activeRow_ then
		...
		if rowInfoCell and not rowInfoCell.disabled and not rowInfoCell.noCommand then
			...
			exit_dialog.setNeedHideHandler("addComboDialogF", addComboDialog)
			local combo					= addComboDialog:show(rowInfo.name, rowInfo.isAxisCommand, deviceName, profileName)
			exit_dialog.setNeedHideHandler("addComboDialogF", nil)
			...
			if combo then
				...
				local prevComboRowInfos = findComboRowsInfo(self, combo, deviceName)
				...
				if rowInfo.isAxisCommand then
					...
					for i, prevComboRowInfo in ipairs(prevComboRowInfos) do
						table.insert(prevComboCommands, InputData.getProfileAxisCommand(profileName, prevComboRowInfo.hash))
					end
				else
					...
					for i, prevComboRowInfo in ipairs(prevComboRowInfos) do
						table.insert(prevComboCommands, InputData.getProfileKeyCommand(profileName, prevComboRowInfo.hash))
					end
				end				
				...
				for i, prevComboCommand in ipairs(prevComboCommands) do
					...
					updateCommandCell(prevComboRowInfos[i])
					updateCategoryValid(self, prevComboRowInfos[i])
				end
		...
		if rowInfoCell then
			...
			for i, combo in ipairs(rowInfoCell.combos) do
				...
				for i, prevComboCommand in ipairs(prevComboCommands) do
					...
					updateCommandCell(prevComboRowInfos[i])
					updateCategoryValid(self, prevComboRowInfos[i])
					updateComboCell(self, self.activeColumn_, prevComboRowInfos[i].rowIndex)
				end
			end
...
local function setModifiers(self)
	...
	local exit_dialog = require('me_exit_dialog')
	exit_dialog.setNeedHideHandler("modifiersDialogF", modifiersDialog)
	local modifiers = modifiersDialog:show(profileName)
	exit_dialog.setNeedHideHandler("modifiersDialogF", nil)
...
local function tuneAxes(self)
	if self.activeColumn_ and self.activeRow_ then
		...
		if rowInfoCell then
			...
			local exit_dialog = require('me_exit_dialog')	
			exit_dialog.setNeedHideHandler("axesTuneDialogF", axesTuneDialog)
			local filters = axesTuneDialog:show(rowInfoCell.combos, deviceName)
			exit_dialog.setNeedHideHandler("axesTuneDialogF", nil)
...
local function tuneForceFeedback(self)
	if self.activeColumn_ then
		...
		if 'joystick' == deviceTypeName then
			...
			local exit_dialog = require('me_exit_dialog')
			exit_dialog.setNeedHideHandler("forceFeedbackTuneDialogF", forceFeedbackTuneDialog)	
			local result = forceFeedbackTuneDialog:show(settings)
			exit_dialog.setNeedHideHandler("forceFeedbackTuneDialogF", nil)	
...
local function construct(self, container, a_w, a_h)
	...
	local buttonWizard = DialogLoader.findWidgetByName(container, "buttonWizard")
	
	buttonWizard:addChangeCallback(function()
		deactivateInput(self)
