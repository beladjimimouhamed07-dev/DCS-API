...
local function createLayer(name,pluginPath)
	-- вызывается из симулятора
	local profileInfo = ProfileDatabase.createProfileInfo(name,pluginPath or '')
...
local function addDebugCommands()
	...
	local commands = {
		...
		-- S renamed to W due to conflict with deck crew
		{combos = {{key = 'W', reformers = {'LShift', 'LCtrl' , 'LAlt' }}}, name = 'Reload Database' ,down = 849}, -- iCommandReloadTables
...
local function reloadMac()
	...
	for i, layerName in ipairs(loadedLayers) do
		...
		if profileInfo then
			-- в МАС только одна раскладка на все самолеты
			if profileInfo.unitName == ProfileDatabase.getMacUnitName() then
