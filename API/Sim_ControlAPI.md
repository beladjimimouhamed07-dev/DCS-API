...
# <a name="intro"></a> Simulation Control User Scripts

The behaviour of the simulator can be altered using the Lua scripts.
You define the hooks to the simulation events, and then do what you want using the provided API.

When loading, the application searches for _$WRITE_DIR/Scripts/Hooks/\*.lua_ files,
...
Each script defines a set of callbacks for the simulator events and sets them with the call
    _Sim.setUserCallbacks(cb_table)_
...
## <a name="example-script"></a> The example user script 'Hooks/test.lua':
    ...
    Sim.setUserCallbacks(test)  -- here we set our callbacks
...
## <a name="lfs-api"></a> Lua File System (lfs) API
  ...
  *     _lfs.currentdir() -> string_
        ...
        Returns the path of the simulator install folder

  *     _lfs.writedir() -> string_

       Returns the path of the current 'Saved Games' sub folder, used to save user data.

  *    _lfs.tempdir() -> string_

       Returns the pat of the application temp folder usually located inside AppData\Local\Temp\
...
## <a name="sim-api"></a> Sim Control API, table 'Sim.\*'

  *    _Sim.setPause(bool)_
  ...
  *    _Sim.getModelTime() -> number_

       returns current simulation time in seconds.

  *    _Sim.getRealTime() -> number_

       returns current real time in seconds relative to the application start time.

  *    _Sim.getMissionOptions() -> table_

      Returns the value of 'mission.options'

  *    _Sim.getMissionDescription() -> string_

      translated mission.descriptionText string

  *    _Sim.getAvailableCoalitions() -> table {_
  ...
  *    _Sim.getAvailableSlots(coalitionID) -> array of {unitId, type, role, callsign, groupName, country}_
  ...
  *    _Sim.getCurrentMission() -> table with the currently loaded mission_

       NOTE: to get valid mission.options use _Sim.getMissionOptions()_

  *    _Sim.getMissionName() -> string_

       Returns the name of the current mission

  *    _Sim.getMissionFilename() -> string_

       Returns the file name of the current mission (returns nil when acting as a multiplayer client).

  *    _Sim.getMissionResult(string side) -> integer [0, 100]_

       Gets mission result for either 'red' or 'blue'

  *    _Sim.getUnitProperty(missionId, propertyId) -> string_

       propertyId:

           Sim.UNIT_RUNTIME_ID, // unique within runtime mission. int
           Sim.UNIT_MISSION_ID, // unique within mission file. int>0
           Sim.UNIT_NAME, // unit name, as assigned by mission designer.
           Sim.UNIT_TYPE, // unit type (Ural, ZU-23, etc)
           Sim.UNIT_CATEGORY,
           Sim.UNIT_GROUP_MISSION_ID, // group ID, unique within mission file. int>0
           Sim.UNIT_GROUPNAME, // group name, as assigned by mission designer.
           Sim.UNIT_GROUPCATEGORY,
           Sim.UNIT_CALLSIGN,
           Sim.UNIT_HIDDEN,// ME hiding
           Sim.UNIT_COALITION,// "blue", "red" or "unknown"
           Sim.UNIT_COUNTRY_ID,
           Sim.UNIT_TASK, //"unit.group.task"
           Sim.UNIT_PLAYER_NAME, // valid for network "humanable" units
           Sim.UNIT_ROLE,//"artillery_commander", "instructor", etc
           Sim.UNIT_INVISIBLE_MAP_ICON,//ME invisible map icon

  *    _Sim.getUnitType(missionId) -> typeId_

       a shortcut for Sim.getUnitProperty(missionId, Sim.UNIT_TYPE)

  *    _Sim.getUnitTypeAttribute(typeId, attr) -> string_

       Returns a value from Database: Objects[typeId][attr],

       For example:

           Sim.getUnitTypeAttribute("Ural", "DisplayName")

  *    _Sim.writeDebriefing(str)_
 ...
 *    _Sim.makeScreenShot(name)_
      
      Makes screenshot with given name.

 *    _Sim.getLogHistory(from) -> {{abstime, level, subsystem, message}, ...}, last_index_
 ...
 *    _Sim.getConfigValue(cfg_path_string)_
...
The API is:
  ...
  *    _log.set_output(log_file_name_wo_ext, rule_subsystem_name, rule_level_mask, rule_output_mode)_
       ...
       rule_level_mask: a sum of log-level bit flags to match messages, valid flags are:
           ...
           log.TRACE - a special level which is excluded from main log file
...
log.\* API is also available from the _$WRITE_DIR/Config/autoexec.cfg_ file so you can control log output in you local machine.
...
What is the difference:
  ...
  *    _log()_ always writes to main log, but may lose messages if the output rate is too high.
  *    _trace()_ output never appears in the main log file, it must be explicitly directed to a log file.
...
[(log.* API, see above)](#log-api)
  ...
  *    _net.dostring_in(state, string) -> string_
      ...
      **OBSOLETE and UNSAFE!!!**
	  ...
	  This API is only allowed in the states listed in the local _$WRITE_DIR/Config/autoexec.cfg_:
      ...
      **NOTE:** There's no need for net.dostring_in anymore.
	  You can return values from a_do_script() mission scripting API directly:
  ...
  *    _net.send_chat_to(string message, playerID to)_
       ...
       Send a direct chat message to a player. There are two special playerIDs: net.CHAT_ALL and net.CHAT_TEAM,
       which send the message to all players or your team correspondingly.
